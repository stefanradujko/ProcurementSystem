 ...
 * @author Stefan
 */
public class VracanjeNarudzbenica extends SistemskaOperacija {
    ...
    protected ServerskiOdgovor commitOperacija(DomenskiObjekat dom) throws Exception {
        ...
        try {
            ...
            so.setOdgovor(lista);
            so.setOperacija(Operacije.VRATI_SVE_NARUDZBENICE);
            so.setPoruka("Sistem je vratio narudžbenice");
            } catch (SQLException e) {
            so.setOperacija(Operacije.VRATI_SVE_NARUDZBENICE);
            so.setPoruka("Sistem ne može da vrati narudžbenice");
            }
       return so;
    }
